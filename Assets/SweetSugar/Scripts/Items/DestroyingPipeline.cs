...
namespace SweetSugar.Scripts.Items
{
    ...
    public class DestroyingPipeline : MonoBehaviour
    {
        ...
        public void DestroyItems(List<MItem> items, Delays delays, Action callback)
        {
        ...
        private IEnumerator DestroyingPipelineCor()
        {
            while (true)
            {
                for (var i = 0; i < pipeline.Count; i++)
                {
                    ...
                    if (bunch.items.Any())
                        MLevelManager.THIS.mLevelData.GetTargetObject().CheckSquares(bunch.items.Select(x => x.square).ToArray());
