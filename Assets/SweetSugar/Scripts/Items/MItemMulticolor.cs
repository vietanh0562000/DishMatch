...
namespace SweetSugar.Scripts.Items
{
    ...
    /// Item multicolor
    /// </summary>
    public class MItemMulticolor : MItem, IItemInterface
    {
        ...
        public override void Check(MItem item1, MItem item2)
        {
            ...
            if (item2.currentType == ItemsTypes.NONE)
            {
                ...
                MLevelManager.THIS.StartCoroutine(SetTypeByColor(item2));
...
//            item2.HideSprites(true);
        ...
        public void Destroy(MItem item1, MItem item2)
        {
            if (item2 == null)
            {
                ...
                if (MLevelManager.GetGameStatus() == GameState.PreWinAnimations)
                {
                    item2 = MLevelManager.THIS.field.squaresArray.First(i => i.mItem != null && i.mItem.currentType == ItemsTypes.NONE).mItem;
                    Check(item1, item2);
                }
                // GetParentItem().StopDestroy();
                if(explodedMItem) DestroyColor(explodedMItem.color);
                else DestroyColor(Random.Range(0,MLevelManager.THIS.mLevelData.colorLimit-1));
        ...
        private IEnumerator SetTypeByColor(MItem item2)
        {
            //		SetTypeByColor(item2.color, item2.currentType);
            var items = MLevelManager.THIS.field.GetItemsByColor(item2.color).Where(i => !i.Equals(GetParentItem()) && i.currentType == ItemsTypes.NONE).ToArray();
            ...
            StartCoroutine(IterateItems(items, item =>
            {
                if (nextType == ItemsTypes.HORIZONTAL_STRIPED || nextType == ItemsTypes.VERTICAL_STRIPED)
                    item.GetComponent<MItem>().NextType = (ItemsTypes)Random.Range(4, 6);
                else
                    item.GetComponent<MItem>().NextType = nextType;
                item.marmaladeTarget = itemMarmaladeTarget;
                item.GetComponent<MItem>().ChangeType(null, false);
//            item.Explodable = false;
        ...
        private void DestroyColor(int p)
        {
            ...
            var items = MLevelManager.THIS.field.GetItemsByColor(p).Where(i => !i.Equals(GetParentItem())).ToArray();
            StartCoroutine(IterateItems(items, item =>
            {
                CreateLightning(transform.position, item.transform.position);
...
//        HideSprites(true);
        ...
        private IEnumerator IterateItems(MItem[] items, Action<MItem> iterateItem, Action onFinished = null)
        {
            var i = 0;
        ...
        {
            MLevelManager.THIS.field.GetItems();
            StartCoroutine(DestroyDoubleBombCor(col, () => { callback(); }));
        }

        private IEnumerator DestroyDoubleBombCor(int col, Action callback)
        {
            for (var i = 0; i < MLevelManager.THIS.field.fieldData.maxCols; i++)
            {
            
                var list = MLevelManager.THIS.GetColumn(i).Where(a => !a.destroying).ToList();
                foreach (var a in list)
                {
                    a.globalExplEffect = true;
                    if (a.currentType == ItemsTypes.MARMALADE) a.GetComponent<MItemMarmalade>().noMarmaladeLaunch = true;
                    CreateLightning(transform.position, a.transform.position);
                    if(jellySpread)
                        a.square?.SetType(SquareTypes.JellyBlock, 1, SquareTypes.NONE, 1);
                }

                MLevelManager.THIS.mLevelData.GetTargetObject().CheckItems(list.ToArray());
