    ...
    public class Square : MonoBehaviour, ISquareItemCommon, IField, IMarmaladeTargetable
    {
        ...
        public MItem MItem
        {
            ...
            set
            {
                if (value == null)
                {
                    if (MLevelManager.THIS.DebugSettings.FallingLog) DebugLogKeeper.Log(mItem + " set square " + name + " empty ", DebugLogKeeper.LogType.Falling);
                }
                else
                {
                    if (MLevelManager.THIS.DebugSettings.FallingLog) DebugLogKeeper.Log(value + " set square " + name, DebugLogKeeper.LogType.Falling);
                }
                mItem = value;
                if(mItem != null)
                {
                    if (GetSubSquare().aboveItem && (mItem.GetSpriteRenderer().sortingOrder >= GetSpriteRenderer().sortingOrder || subSquares.Count>1 && subSquares[subSquares
                    .Count-2].GetComponent<SpriteRenderer>().sortingOrder >= mItem.GetSpriteRenderer().sortingOrder))
                    {
                        GetSpriteRenderer().sortingOrder += 2;
                        mItem.GetComponent<IItemInterface>().SetOrder(GetSpriteRenderer().sortingOrder - 2);
                    }
                    else if (!GetSubSquare().aboveItem && mItem.GetSpriteRenderer().sortingOrder <= GetSpriteRenderer().sortingOrder)
                    {
                        mItem.GetComponent<IItemInterface>().SetOrder( GetSpriteRenderer().sortingOrder +1);
                    }
                }

                if (mItem == null && isEnterPoint && MLevelManager.GetGameStatus() != GameState.RegenLevel) StartCoroutine(CheckNullItem());
            }
        }
        ...
        void Start()//init some objects
        {
            ...
            if ((MLevelData.THIS.IsTargetByActionExist(CollectingTypes.ReachBottom) || direction != Vector2.down) && enterSquare && type != SquareTypes.NONE)
            {
            ...
            isSquareToClear = MLevelManager.THIS.mLevelData.IsTargetByActionExist(CollectingTypes.Clear) && MLevelManager.THIS.mLevelData.GetTargetSprites().Any(i => i.name == 
        ...
        IEnumerator CheckNullItem()
        {
            while (mItem == null)
            {
                if (!IsHaveSolidAbove() && !IsHaveItemsAbove() && isEnterPoint && mItem == null)
                    GenItem();
        ...
        public MItem GenItem(bool falling = true, ItemsTypes itemType = ItemsTypes.NONE, int color = -1, ItemForEditor squareBlockItem=null)
        {
            if (IsNone() && !CanGoInto())
                return null;
            ...
            {
                if (MLevelData.THIS.IsTargetByActionExist(CollectingTypes.ReachBottom) && !IsObstacle() && !field.IngredientsByEditor)
                    item = GetIngredientItem();
                if(!item)
                    item = ObjectPooler.Instance.GetPooledObject("Item", this);
                if (MLevelManager.THIS.DebugSettings.FallingLog) DebugLogKeeper.Log(item + " type by " + itemType, DebugLogKeeper.LogType.Falling, true );

            }
            else
            {
                item = ObjectPooler.Instance.GetPooledObject(itemType.ToString(), this);
                item?.GetComponent<MItem>().anim?.SetTrigger("bonus_appear");
            }
            if (item == null)
            {
            ...
            MItem mItemComponent = item.GetComponent<MItem>();
            if(MLevelManager.THIS.mLevelData.GetTargetCounters().Any(i=>i.collectingAction==CollectingTypes.Destroy && i.extraObject==mItemComponent.GetSpriteRenderer().sprite))
                item.AddComponent<TargetComponent>();
            mItemComponent.square = this;
            mItemComponent.CheckPlusFive();
            ...
            if (color == -1)
                mItemComponent.GenColor();
            else if (colorableComponent != null) colorableComponent.SetColor(color); 
            mItemComponent.field = field;
            mItemComponent.needFall = falling;
            if(!falling)
            {
                item.transform.position = (Vector3) ((Vector2) transform.position) + Vector3.back * 0.2f;

            }
            if (MLevelManager.THIS.gameStatus != GameState.Playing && MLevelManager.THIS.gameStatus != GameState.Tutorial)
                MItem = mItemComponent;
            ...
            if (falling)
            {
                ...
                mItemComponent.JustCreatedItem = true;

            }
            if (MLevelManager.THIS.DebugSettings.DestroyLog) DebugLogKeeper.Log(name + " gen item " + item.name + " pos " + item.transform.position, DebugLogKeeper.LogType.Destroying);

            mItemComponent.needFall = falling;
//        if ((!nextSquare?.IsFree() ?? false) || Item == itemComponent)
            mItemComponent.StartFallingTo(mItemComponent.GenerateWaypoints(this));
//            itemComponent.StartFalling();
        ...
        {
            GameObject item = null;
            var allIngredientTargets = MLevelManager.THIS.mLevelData.GetTargetCounters()
            ...
            var fieldCount = MLevelManager.THIS.field.GetItems()?.Where(i => selectedTarget != null && selectedTarget.extraObjects.Any(x=>x== i.GetSprite()))?.Count() ?? 0;
            fieldCount += MLevelManager.THIS.animateItems.Where(i => selectedTarget != null && selectedTarget.extraObjects.Any(x=>x== i.GetComponent<SpriteRenderer>().sprite )).Count();
            var spawnAmount = selectedTarget.targetPrefab.GetComponent<MItem>().SpawnAmount.SpawnAmount;
            if (fieldCount < spawnAmount)
            {
                if (!IsBottom())
                {
                    ...
                    {
                        if (Random.Range(0, MLevelManager.THIS.mLevelData.limit / 3) == 0)
                        {
                            item = ObjectPooler.Instance.GetPooledObject(selectedTarget.targetPrefab.name, this);
                            item.GetComponent<MItem>().sprRenderer[0].sprite = selectedTarget.extraObject;
                        }
                    }
                }
            }
...
//        if (!nextSq?.IsNone() ?? false)
            ...
            if (type != _type && !IsTypeExist(_type) && IsAvailable())
            {
                for (var i = 0; i < sqLayer; i++)
                {
                    ...
                    if (prefab != null && _type != SquareTypes.EmptySquare && _type != SquareTypes.NONE)
                    {
                        ...
                        MLevelManager.THIS.mLevelData.SetSquareTarget(b, _type, prefab);
        ...
        public void CreateObstacle(SquareTypes obstacleType, int obLayer)
        {
            ...
            for (var i = 0; i < obLayer; i++)
            {
                ...
                b.transform.localScale = Vector2.one;
                MLevelManager.THIS.mLevelData.SetSquareTarget(b, obstacleType, prefab);
        ...
        void GenSpiral()
        {
            MLevelManager.OnLevelLoaded += () =>
            {
                if (MItem != null) MItem.transform.position = transform.position;
            };
        ...
        public bool AnyFallingItemsInSeq(int color = -1)
        {
            ...
            if (color > -1)
            {
                var anyFallingItemsInSeq = sq.Any(i => i.MItem != null && i.MItem?.color == color && i.MItem.falling);
                return anyFallingItemsInSeq;
            }

            return sq.Any(i => i.MItem?.falling ?? false);
        }
        /// <summary>
        ...
        public MItem GetItemAbove()
        {
            ...
            {
                if (square.IsFree() && square.MItem)
                    return square.MItem;
                if (!square.IsFree())
                    return null;
            }
            if (list.Count() == 0) return GetPreviousSquare().MItem;
            return null;
        }
        /// <summary>
        ...
        {
            var matches = 0;
            if ((GetNeighborBottom()?.MItem?.color ?? -1) == col)
                matches=1;
            if (((GetNeighborBottom()?.GetNeighborBottom())?.MItem?.color ?? -1) == col)
                matches++;
            if ((GetNeighborTop()?.MItem?.color ?? -1) == col)
                matches=1;
            if (((GetNeighborTop()?.GetNeighborTop())?.MItem?.color ?? -1) == col)
                matches++;
            if ((GetNeighborRight()?.MItem?.color ?? -1) == col)
                matches=1;
            if (((GetNeighborRight()?.GetNeighborRight())?.MItem?.color ?? -1) == col)
                matches++;
            if ((GetNeighborLeft()?.MItem?.color ?? -1) == col)
                matches=1;
            if (((GetNeighborLeft()?.GetNeighborLeft())?.MItem?.color ?? -1) == col)
                matches++;
            return matches;
        }
        /// <summary>
        ...
        Hashtable FindMoreMatches(int spr_COLOR, Hashtable countedSquares, FindSeparating separating, Hashtable countedSquaresGlobal = null)
        {
            ...
            if (MItem == null || MItem.destroying || MItem.falling)
                return countedSquares;
            //    if (LevelManager.This.countedSquares.ContainsValue(this.item) && globalCounter) return countedSquares;
            if (MItem.color == spr_COLOR && !countedSquares.ContainsValue(MItem) && MItem.currentType != ItemsTypes.INGREDIENT && MItem.currentType != ItemsTypes.MULTICOLOR && !MItem
                    .falling && MItem.Combinable)
            {
                if (MLevelManager.THIS.onlyFalling && MItem.JustCreatedItem)
                    countedSquares.Add(countedSquares.Count - 1, MItem);
                else if (!MLevelManager.THIS.onlyFalling)
                    countedSquares.Add(countedSquares.Count - 1, MItem);
                else
                    return countedSquares;
        ...
        public List<MItem> FindMatchesAround(FindSeparating separating = FindSeparating.NONE, int matches = 3, Hashtable countedSquaresGlobal = null)
        {
            var globalCounter = true;
            var newList = new List<MItem>();
            if (countedSquaresGlobal == null)
            {
                globalCounter = false;
                countedSquaresGlobal = new Hashtable();
            ...
            //		{
            countedSquares = FindMoreMatches(MItem.color, countedSquares, FindSeparating.VERTICAL, countedSquaresGlobal);
            //		}
        ...
        {
            if (MLevelManager.THIS.StopFall) return;
            if (MItem != null && CanGoOut())
            {
                var nxtSq = Vector2.Distance(this.transform.position,MItem.transform.position)>0.3f ? this: GetNextSquareRecursively(this);//GetNextSquareRecursively();
                if (nxtSq)
                {
                    if (nxtSq.CanGoInto())
                    {
                        if (nxtSq.MItem == null)
                        {
                            MItem.ReplaceCurrentSquareToFalling(nxtSq);
                        }
                    }
                }
            }
        ...
        public Square GetNextSquareRecursively(Square originSquare)
        {
            ...
            if (sq != null)
            {
                if (sq.IsNone())
                    return this;
                if (sq.MItem != null || !sq.CanGoInto() || (Vector2.Distance(sq.GetPosition(), GetPosition())>=2 && originSquare.orderInSequence-sq.orderInSequence>1 ))
                    return this;
        ...
        public void DestroyBlock(bool destroyTarget=false)
        {
            ...
            {
                destroyIteration = MLevelManager.THIS.destLoopIterations;
            ...
            if (GetSubSquare().CanGoInto())
            {
                ...
                {
                    if (!sq.GetSubSquare().CanGoInto() || (sq.MItem?.currentType == ItemsTypes.SPIRAL && MItem?.currentType != ItemsTypes.SPIRAL))
                        sq.DestroyBlock();
                }
            }

            if (MItem?.currentType == ItemsTypes.SPIRAL)
            {
                MItem.DestroyItem();
            }
            ...
            if (subSquares.Count > 0)
            {
                ...
                if (subSquare.GetComponent<TargetComponent>() == null)
                {
                    ...
                    if (MLevelManager.THIS.animateItems.Any(i => i.linkObjectHash == gameObject.GetHashCode())) return;
                    var itemAnim = new GameObject();
                    var animComp = itemAnim.AddComponent<AnimateItems>();
                    MLevelManager.THIS.animateItems.Add(animComp);
                ...
                MLevelManager.THIS.mLevelData.GetTargetObject().CheckSquare(new[] { subSquare });
                MLevelManager.THIS.ShowPopupScore(subSquare.score, transform.position, 0);
                ...
                if (subSquare.CanGoInto() && MItem == null) MItem = null;
            ...
            if (IsFree() && MItem == null) MItem = null;
        }

        private IEnumerator CheckClearTarget()
        {
            yield return new WaitForSeconds(1.5f);
            if(GetSubSquare().IsCleared()) MLevelManager.THIS.mLevelData.GetTargetsByAction(CollectingTypes.Clear).ForEachY(i=>
            {
        ...
        public bool IsHaveFallingItemsAbove()
        {
            ...
            foreach (var square in seq)
            {
                if (square.MItem && !square.MItem.falling && !square.MItem.needFall)
                    return true;
            }
            return false;
        }
        ...
        private void SetBorderDirection(Vector2 dir)
        {
            ...
            border.transform.localPosition = Vector2.zero + dir * (MLevelManager.THIS.squareWidth + 0.1f);
