    ...
    public class LevelMakerEditor : EditorWindow
    {
        ...
        public int SubLevelNumber
        {
            ...
            set
            {
                ...
                _mLevelData.currentSublevelIndex = subLevelNumber - 1;
            }
        }
        ...
        private void OnFocus()
        {
            ...
            if (_mLevelData != null)
            {
                if (_mLevelData.GetField(subLevelNumber - 1).maxRows <= 0)
                    _mLevelData.GetField(subLevelNumber - 1).maxRows = 9;
                if (_mLevelData.GetField(subLevelNumber - 1).maxCols <= 0)
                    _mLevelData.GetField(subLevelNumber - 1).maxCols = 9;
                Initialize();
            }
            ...
            if (EditorSceneManager.GetActiveScene().name == "game")
            {
                ...
                MLevelManager lm = Camera.main.GetComponent<MLevelManager>();
        ...
        private void Initialize()
        {
            ...
            var num = 0;
            var simpleItem = Resources.Load<GameObject>("Items/Item").GetComponent<MItemSimple>();
            ...
            var bonusitems = Resources.FindObjectsOfTypeAll(typeof(EditorIcon))
                ...
                .Select(i => new ItemForEditor
                {
                    ...
                    Texture = i.GetComponent<MItem>().sprRenderer.FirstOrDefault().sprite.texture,
                    order = 100
                });
            ...
            itemsForEditor = simpleitems.Union(bonusitems.GroupBy(i => i.ItemType).SelectMany(i => i))
                ...
                .ToArray();
            if (_mLevelData.target.prefabs.All(i => i.GetComponent<IColorableComponent>()))
            {
                for (var index = 0; index < _mLevelData.subTargetsContainers.Count; index++)
                {
                    var levelDataSubTargetsContainer = _mLevelData.subTargetsContainers[index];
        ...
        {
            _mLevelData.GetField(subLevelNumber - 1).levelSquares =
                new SquareBlocks[_mLevelData.GetField(subLevelNumber - 1).maxCols *
                                 _mLevelData.GetField(subLevelNumber - 1).maxRows];
            for (int i = 0; i < _mLevelData.GetField(subLevelNumber - 1).levelSquares.Length; i++)
            {
                SquareBlocks sqBlocks = new SquareBlocks();
                sqBlocks.block = SquareTypes.EmptySquare;
                sqBlocks.obstacle = SquareTypes.NONE;

                _mLevelData.GetField(subLevelNumber - 1).levelSquares[i] = sqBlocks;
            }
            ResetDirection();
        }
...
//        GUILayout.Space(30);
            ...
            if (selected == 0 && _mLevelData.fields.Any())
            {
                if (_mLevelData != null)
                {
                    //                if (EditorSceneManager.GetActiveScene().name == "game")
                    {
                        ...
                        GUILayout.Space(70);
                        _mLevelData.GetField(subLevelNumber - 1).bombTimer = EditorGUILayout.IntField("",
                            _mLevelData.GetField(subLevelNumber - 1).bombTimer, GUILayout.Width(50));
                        GUILayout.EndHorizontal();
                        GUILayout.Space(10);
...
//        }
        ...
        {
            MLevelManager lm = Camera.main.GetComponent<MLevelManager>();//TODO: move all game settings to scriptabble
            InitScript initscript = Camera.main.GetComponent<InitScript>();
...
//            GUILayout.Space(60);
        ...
        private void GUILevelSize()
        {
            ...
            GUILayout.BeginVertical();
            GUILayout.BeginHorizontal();
            int oldValue = _mLevelData.GetField(subLevelNumber - 1).maxRows + _mLevelData.GetField(subLevelNumber - 1).maxCols;
            GUILayout.Label("Columns", GUILayout.Width(50));
            GUILayout.Space(100);
            _mLevelData.GetField(subLevelNumber - 1).maxCols = EditorGUILayout.IntField("",
                _mLevelData.GetField(subLevelNumber - 1).maxCols, GUILayout.Width(50));
            GUILayout.EndHorizontal();
            GUILayout.BeginHorizontal();
            GUILayout.Label("Rows", GUILayout.Width(50));
            GUILayout.Space(100);
            _mLevelData.GetField(subLevelNumber - 1).maxRows = EditorGUILayout.IntField("",
                _mLevelData.GetField(subLevelNumber - 1).maxRows, GUILayout.Width(50));
            GUILayout.EndHorizontal();
            GUILayout.EndVertical();
            GUILayout.EndHorizontal();

            if (_mLevelData.GetField(subLevelNumber - 1).maxRows < 3)
                _mLevelData.GetField(subLevelNumber - 1).maxRows = 3;
            if (_mLevelData.GetField(subLevelNumber - 1).maxCols < 3)
                _mLevelData.GetField(subLevelNumber - 1).maxCols = 3;
            if (_mLevelData.GetField(subLevelNumber - 1).maxRows > 11)
                _mLevelData.GetField(subLevelNumber - 1).maxRows = 11;
            if (_mLevelData.GetField(subLevelNumber - 1).maxCols > 11)
                _mLevelData.GetField(subLevelNumber - 1).maxCols = 11;
            if (oldValue != _mLevelData.GetField(subLevelNumber - 1).maxRows + _mLevelData.GetField(subLevelNumber - 1).maxCols)
            {
                Initialize();
                InitializeSublevel();
                ClearLevel();
        ...
        private void GUILimit()
        {
            GUILayout.BeginHorizontal();
            {
                ...
                GUILayout.Space(100);
                LIMIT limitTypeSave = _mLevelData.limitType;
                int oldLimit = _mLevelData.limit;
                _mLevelData.limitType = (LIMIT)EditorGUILayout.EnumPopup(_mLevelData.limitType, GUILayout.Width(93));
                if (_mLevelData.limitType == LIMIT.MOVES)
                    _mLevelData.limit = EditorGUILayout.IntField(_mLevelData.limit, GUILayout.Width(50));
                else
                {
                    GUILayout.BeginHorizontal();
                    int limitMin = EditorGUILayout.IntField(_mLevelData.limit / 60, GUILayout.Width(30));
                    GUILayout.Label(":", GUILayout.Width(10));
                    int limitSec =
                        EditorGUILayout.IntField(_mLevelData.limit - (_mLevelData.limit / 60) * 60, GUILayout.Width(30));
                    _mLevelData.limit = limitMin * 60 + limitSec;
                    GUILayout.EndHorizontal();
                }

                if (_mLevelData.limit <= 0)
                    _mLevelData.limit = 1;
                if (limitTypeSave != _mLevelData.limitType || oldLimit != _mLevelData.limit)
                    dirtyLevel = true;
                // 	SaveLevel();
            }
            GUILayout.EndHorizontal();
        ...
        private void GUIColorLimit()
        {
            ...
            int saveInt = _mLevelData.colorLimit;
            GUILayout.Label("Color limit", EditorStyles.label, GUILayout.Width(100));
            GUILayout.Space(50);
            _mLevelData.colorLimit = (int)GUILayout.HorizontalSlider(_mLevelData.colorLimit, 3, 6, GUILayout.Width(100));
            _mLevelData.colorLimit = EditorGUILayout.IntField("", _mLevelData.colorLimit, GUILayout.Width(50));
            if (_mLevelData.colorLimit < 3)
                _mLevelData.colorLimit = 3;
            if (_mLevelData.colorLimit > 6)
                _mLevelData.colorLimit = 6;

            GUILayout.EndHorizontal();

            if (saveInt != _mLevelData.colorLimit)
            {
                dirtyLevel = true;
                // SaveLevel();
            }
...
//        GUILayout.Space(35);
            ...
            GUILayout.BeginHorizontal();
            int s = 0;
            s = EditorGUILayout.IntField("", _mLevelData.star1, GUILayout.Width(50));
            if (s != _mLevelData.star1)
            {
                _mLevelData.star1 = s;
                dirtyLevel = true;
                // SaveLevel();
            ...
            if (_mLevelData.star1 <= 0)
                _mLevelData.star1 = 100;
            s = EditorGUILayout.IntField("", _mLevelData.star2, GUILayout.Width(50));
            if (s != _mLevelData.star2)
            {
                _mLevelData.star2 = s;
                dirtyLevel = true;
                // SaveLevel();
            ...
            if (_mLevelData.star2 < _mLevelData.star1)
                _mLevelData.star2 = _mLevelData.star1 + 10;
            s = EditorGUILayout.IntField("", _mLevelData.star3, GUILayout.Width(50));
            if (s != _mLevelData.star3)
            {
                _mLevelData.star3 = s;
                dirtyLevel = true;
                // SaveLevel();
            ...
            if (_mLevelData.star3 < _mLevelData.star2)
            {
                _mLevelData.star3 = _mLevelData.star2 + 10;
                dirtyLevel = true;
                // SaveLevel();
            }
        ...
        private void GUIMamalade()
        {
            GUILayout.BeginHorizontal();
            {
                ...
                bool s = false;
                s = EditorGUILayout.Toggle(_mLevelData.enableMarmalade, GUILayout.Width(50));
                if (s != _mLevelData.enableMarmalade)
                {
                    _mLevelData.enableMarmalade = s;
                    dirtyLevel = true;
                    // SaveLevel();
                }
            }
        ...
        private void GUINoRegen()
        {
            GUILayout.BeginHorizontal();
            {
                ...
                bool s = false;
                GUILayout.Space(70);
                s = EditorGUILayout.Toggle(_mLevelData.GetField().noRegenLevel, GUILayout.Width(50));
                if (s != _mLevelData.GetField().noRegenLevel)
                {
                    _mLevelData.GetField().noRegenLevel = s;
                    dirtyLevel = true;
                    // SaveLevel();
                }
            }
            ...
            void UpdateTarget(int newselectedTarget, TargetLevel targetLevel, TargetContainer target)
            {
                ...
                targetLevel.ChangeTarget(target, newselectedTarget, _mLevelData, targetEditorScriptable);
                dirtyLevel = true;
            }
        ...
        private void GUITarget()
        {
                ...
                // if (target_settings)
                {
                    GUILayout.BeginVertical();
                    {
                        ...
                        var targetObjects = subTargets.GroupBy(i => i.targetType.type).FirstOrDefault();
                            ...
                            GUILayout.BeginHorizontal();
                            {
                                newselectedTarget = EditorGUILayout.Popup(selectedTarget, _mLevelData.GetTargetsNames(),
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                GUILayout.Space(103);
                int newselectedTutorial = EditorGUILayout.Popup(_mLevelData.selectedTutorial, GetTutorialNames(),GUILayout.Width(160),GUILayout.ExpandWidth (false));
                if(_mLevelData.selectedTutorial != newselectedTutorial)
                {
                    _mLevelData.selectedTutorial = newselectedTutorial;
                    dirtyLevel = true;
                }
            }
            GUILayout.EndHorizontal();
        ...
        {
            var targets = _mLevelData.GetTargetsNames();
            var currentTarget = _mLevelData.target.name;
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                GUILayout.BeginVertical();
                {
                        ...
                        GUILayout.Height(50)))
                    {
                        ...
                        for (int i = 0; i < _mLevelData.GetField(subLevelNumber - 1).levelSquares.Length; i++)
                        {
                            _mLevelData.GetField(subLevelNumber - 1).levelSquares[i].isEnterTeleport = false;
                            _mLevelData.GetField(subLevelNumber - 1).levelSquares[i].teleportCoordinatesLinkedBack =
                                Vector2Int.one * -1;
                            _mLevelData.GetField(subLevelNumber - 1).levelSquares[i].teleportCoordinatesLinked =
                                Vector2Int.one * -1;
                        }
                ...
                GUILayout.BeginVertical();
                {
                    ...
                    GUILayout.BeginHorizontal();
                    {
                            ...
                            GUILayout.Height(50)))
                        {
                            ...
                            var squares = _mLevelData.GetField(subLevelNumber - 1).levelSquares;
        ...
        {
            _mLevelData.GetField(subLevelNumber - 1).levelSquares.ToList().ForEach(i => i.enterSquare = false);
            if (direction == Vector2.down)
            {
                for (int col = 0; col < _mLevelData.GetField(subLevelNumber - 1).maxCols; col++)
                {
                    var squareBlocks = _mLevelData.GetBlock(0, col);
            ...
            if (direction == Vector2.up)
            {
                for (int col = 0; col < _mLevelData.GetField(subLevelNumber - 1).maxCols; col++)
                {
                    var squareBlocks = _mLevelData.GetBlock(_mLevelData.GetField(subLevelNumber - 1).maxRows - 1, col);
                    if (squareBlocks.teleportCoordinatesLinkedBack == Vector2Int.one * -1)
                        squareBlocks.enterSquare = true;
                }
            }
            ...
            {
                for (int row = 0; row < _mLevelData.GetField(subLevelNumber - 1).maxRows; row++)
                {
                    var squareBlocks = _mLevelData.GetBlock(row, _mLevelData.GetField(subLevelNumber - 1).maxCols - 1);
                    if (squareBlocks.teleportCoordinatesLinkedBack == Vector2Int.one * -1)
                        squareBlocks.enterSquare = true;
                }
            }
            ...
            if (direction == Vector2.right)
            {
                for (int row = 0; row < _mLevelData.GetField(subLevelNumber - 1).maxRows; row++)
                {
                    var squareBlocks = _mLevelData.GetBlock(row, 0);
                    if (squareBlocks.teleportCoordinatesLinkedBack == Vector2Int.one * -1)
                        squareBlocks.enterSquare = true;
                }
            }
        ...
        private void ClearItems()
        {
            for (int i = 0; i < _mLevelData.GetField(subLevelNumber - 1).levelSquares.Length; i++)
            {
                _mLevelData.GetField(subLevelNumber - 1).levelSquares[i].item = null;
            }
...
//                                if (HideAnotherTargetBlock(squareTypeItem.ToString())) continue;
        ...
        { 
            layers = 1;
            layer = 0;
            foreach (var squareBlocks in _mLevelData.GetField(subLevelNumber - 1).levelSquares)
            {
        ...
        private void FillLevel()
        {
            for (int i = 0; i < _mLevelData.GetField(subLevelNumber - 1).levelSquares.Length; i++)
            {
                var squareBlocks = _mLevelData.GetField(subLevelNumber - 1).levelSquares[i];
        ...
        private void GUIGameField()
        {
            ...
            for (int row = 0; row < GetField().maxRows; row++)
            {
                ...
                for (int col = 0; col < GetField().maxCols; col++)
                {
                    ...
                    SquareBlocks squareBlock = _mLevelData.GetBlock(row, col);
                    ...
                    if (GUILayout.Button(imageButton as Texture, GUILayout.Width(50), GUILayout.Height(50)))
                    {
                        ...
                        else if (section == 2)
                        {
                            if (selectDirectionType == 0)
                                ...
                                _mLevelData.GetBlock(row, col).enterSquare = !_mLevelData.GetBlock(row, col).enterSquare;
                        }
                        else if (section == 3)
                        {
                            if (squareBlock.isEnterTeleport ||
                            ...
                            {
                                _mLevelData.GetBlock(row, col).enterSquare = false;
        ...
        private FieldData GetField() => _mLevelData.GetField(subLevelNumber - 1);
        ...
        {
            if (section != 1) return;
            var marmalades = _mLevelData.GetField().levelSquares.Where(i => i.item != null && i.item.EnableMarmaladeTargets);
            ...
            foreach (var marmalade in marmalades)
            {
                ...
                {
                    Handles.DrawLine(marmalade.guiRect.center, _mLevelData.GetBlock(targetPosition).guiRect.center);
                    RotateGizmo(marmalade.guiRect.center, _mLevelData.GetBlock(targetPosition).guiRect.center, 20f);
                    RotateGizmo(marmalade.guiRect.center, _mLevelData.GetBlock(targetPosition).guiRect.center, -20f);
                }
                //            Handles.EndGUI();
            }
                ...
                for (int col = 0; col < _mLevelData.GetField(subLevelNumber - 1).maxCols; col++)
                {
                    ...
                    if (section == 2 || section == 3)
                    {
                        if (squareBlock.teleportCoordinatesLinkedBack != Vector2Int.one * -1)
                        {
                            ...
                            Vector2 position2 = _mLevelData.GetBlock(squareBlock.teleportCoordinatesLinkedBack).guiRect
                                .center;
        ...
        private void AddLevel()
        {
            ...
            SubLevelNumber = 1;
            ScriptableLevelManager.CreateFileLevel(levelNumber, _mLevelData);
            _mLevelData = ScriptableLevelManager.LoadLevel(levelNumber);
            Initialize();
            ClearLevel();
            ClearItems();
            SaveLevel(levelNumber);
        ...
        {
            int lastLevel = MLoadingManager.GetLastLevelNum();
        ...
        {
            return _mLevelData.fields.Count; //GetSublevelsScriptable();
        }
        ...
        private void DeleteBlock(SquareBlocks squareBlock, Square squarePrototype)
        {
            ...
            if(squareBlock.blocks[index].squareType==SquareTypes.BigBlock)
            {
                ...
                for (int x = originalPos.x; x < originalPos.x+size.x; x++)
                {
                    ...
                    {
                        var originalBlock = _mLevelData.GetBlock(y,x);
            ...
            var sum = _mLevelData.GetField().levelSquares.Max(i=>i.blocks.Count);
            if(sum< layers)
                layers = sum;
        ...
        private void AddBlock(SquareBlocks squareBlock, Square squarePrototype)
        {
            ...
            for (int x = 0; x < xMax; x++)
            {
                ...
                {
                    list.Add( _mLevelData.GetBlock(squareBlock.position.y + y, squareBlock.position.x + x));
                }
            }
        ...
        {
            _mLevelData.InitTargetObjects();
            SquareBlocks[] levelSquares = _mLevelData.GetField(subLevelNumber - 1).levelSquares;
            ...
            if (!enterSquaresExist)
            {
                for (int col = 0; col < _mLevelData.GetField(subLevelNumber - 1).maxCols; col++)
                {
                    _mLevelData.GetBlock(0, col).enterSquare = true;
                }
            }

            if (levelScriptable.levels.Count() < _levelNumber)
                levelScriptable.levels.Add(_mLevelData);
            else
                levelScriptable.levels[_levelNumber - 1] = _mLevelData.DeepCopy(_levelNumber);
            ScriptableLevelManager.SaveLevel(levelPath, _levelNumber, _mLevelData);
