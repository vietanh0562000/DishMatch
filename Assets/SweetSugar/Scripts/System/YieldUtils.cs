...
namespace SweetSugar.Scripts.System
{
    public class WaitWhileDestroyPipeline : CustomYieldInstruction
    {
        ...
        public WaitWhileDestroyPipeline(List<MItem> destroyItems, Delays delays)
        {
...
//        }
            items = MLevelManager.THIS.field.GetItems(false, null, false);
        }
    }
    ...
    public class WaitWhileCollect : CustomYieldInstruction
    {
        ...
        {
            items = MLevelManager.THIS.animateItems.Where(i=>i.target).ToArray();
        }
    }
    ...
    public class WaitWhileFallSide : CustomYieldInstruction
    {
        ...
        public override bool keepWaiting
        {
            ...
            {
                var ii = items.WhereNotNull().Where(i=>i.gameObject.activeSelf).Any(i => i.falling);
                var squares = MLevelManager.THIS.field.squaresArray.Where(i=>i.isEnterPoint && i.IsFree() && i.MItem == null).ToArray();
                if (ii && squares.Any())
                {
        ...
        {
            items = MLevelManager.THIS.field.GetItems(false, null, false).Where(i => i.square).OrderBy(i => i.square.orderInSequence).ToList();
    ...
    public class WaitForSubLevelChange : CustomYieldInstruction
    {
        ...
        public override bool keepWaiting
        {
            get
            {
                if (nextMove)
                {
                    MLevelManager.OnSublevelChanged -= OnSublevelChanged;
                    return false;
                }
    ...
    public class WaitWhileAnimations : CustomYieldInstruction
    {
        ...
        {
            var destroyingItems = MLevelManager.THIS.field.GetDestroyingItems();
            var i = 0;
        ...
        {
            MLevelManager.THIS.StartCoroutine(ienumerator);
        }
