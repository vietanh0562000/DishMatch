 ...
 namespace SweetSugar.Scripts.Level
{
    ...
    /// </summary>
    [Serializable]
    public class MLevelData
    {
        public string Name;
        public static MLevelData THIS;
        /// level number
        public int levelNum;
        ...
        public MLevelData(bool isPlaying, int currentLevel)
        {
        ...
        public void SetTarget(int index)
        {
            ...
            try
            {
                ...
                if(subTargetPrefabs.Length>1)
                {
                    ...
                    {
                        var component = _target.GetComponent<MItem>();
        ...
        public void SetItemTarget(MItem mItem)
        {
            foreach (var _subTarget in TargetCounters)
            {
                if (_subTarget.targetPrefab && mItem.CompareTag(_subTarget.targetPrefab.tag) && _subTarget.count > 0 && mItem.gameObject.GetComponent<TargetComponent>() == null)
                {
                    mItem.gameObject.AddComponent<TargetComponent>();
                }
            }
        }
        ...
        public MLevelData DeepCopy(int level)
        {
            LoadTargetObject();
            var other = (MLevelData)MemberwiseClone();
        ...
        public MLevelData DeepCopyForPlay(int level)
        {
            MLevelData data = DeepCopy(level);
            THIS = data;
            return data;
        }
