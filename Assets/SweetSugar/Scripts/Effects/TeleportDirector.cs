...
namespace SweetSugar.Scripts.Effects
{
    ...
    public class TeleportDirector : MonoBehaviour
    {
        ...
        public void StartTeleport(MItem mItem, Action callback)
        {
            ...
            director.Play();
            StartCoroutine(Wait(mItem, callback));
        }

        IEnumerator Wait(MItem mItem, Action callback)
        {
            yield return new WaitUntil(() => director.time >= endKey);
            if (mItem?.anim != null)
                mItem.anim.enabled = true;
