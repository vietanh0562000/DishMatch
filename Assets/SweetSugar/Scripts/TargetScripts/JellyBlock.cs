...
//        return LevelManager.THIS.fieldBoards.SelectMany(i => i.squaresArray).WhereNotNull().Count(i => i.type == SquareTypes.JellyBlock);
            return MLevelManager.THIS.fieldBoards.SelectMany(i => i.squaresArray).WhereNotNull().SelectMany(i => i.subSquares).Count(i => i.type == SquareTypes.JellyBlock);
        }

        public override void InitTarget(MLevelData mLevelData)
        {
            description = "Spread the jelly everywhere";
            subTargetContainers[0].count = mLevelData.fields.Sum(x => x.levelSquares.WhereNotNull().Count(i => i.block == SquareTypes.JellyBlock));
        ...
        {
            var count = 0;
            var field = MLevelManager.THIS.field;
        ...
        {
            return MLevelManager.THIS.field.squaresArray.WhereNotNull().Count(i => i.type != SquareTypes.NONE && !i.undestroyable);
        }
        ...
        {
            return MLevelManager.THIS.fieldBoards.SelectMany(i => i.squaresArray).WhereNotNull().Count(i => i.type != SquareTypes.NONE && !i.undestroyable);
        }
