 ...
 namespace SweetSugar.Scripts.Level
 {
     ...
     public class FieldBoard : MonoBehaviour
     {
         ...
         public void CreateField()
         {
             ...
             if (MLevelManager.THIS.enableMarmalade)
                 CreateMarmalade();
//        new OutlineBorder(fieldData.maxRows, fieldData.maxCols, this);
         ...
         private void CreateTestItems()
         {
             ...
             GetSquare(3, 3).MItem.colorableComponent.SetColor(0);
             GetSquare(3, 4).MItem.colorableComponent.SetColor(0);
             GetSquare(4, 3).MItem.colorableComponent.SetColor(0);
             GetSquare(4, 4).MItem.colorableComponent.SetColor(0);
         }
         ...
         private IEnumerator DeleteMatches()
         {
             ...
             do
             {
                 combs = MLevelManager.THIS.CombineManager.GetCombines(this, out allFoundCombines);
...
//            yield return new WaitForEndOfFrame();
             ...
             MLevelManager.THIS.gameStatus = GameState.WaitForPopup;
         }
         ...
         public void ChangeFoundCombines(List<List<MItem>> allFoundCombines)
         {
         ...
         public void GenerateNewItems(bool falling = true)
         {
             ...
             foreach (var square in squares)
             {
                 ...
                 {
                     if (square.MItem == null) //|| !falling && square.item?.currentType != ItemsTypes.SPIRAL)
                     {
         ...
         public void RegenItems(bool falling = true)
         {
             ...
             foreach (var square in squares)
             {
                 ...
                 {
                     if (square.MItem != null && square.MItem.currentType == ItemsTypes.NONE) //|| !falling && square.item?.currentType != ItemsTypes.SPIRAL)
                     {
                         square.MItem.colorableComponent.RandomizeColor();
                     }
                 }
             }
         }
         ...
         public List<MItem.Waypoint> GetWaypoints(Square startSquare, Square destSquare, List<Square> list = null)
         {
             if (destSquare.Equals(startSquare) || startSquare == null)
                 return list.Select(i => new MItem.Waypoint(i.transform.position, i)).ToList();
         ...
         public void DestroyItemsAround(Square square, MItem item1)
         {
             var itemsList = MLevelManager.THIS.GetItemsAround9(square);
             ...
             MLevelManager.THIS.GetSquaresAroundSquare(square).ForEach(i => i.DestroyBlock());
         ...
         public List<List<MItem>> GetMatches(FindSeparating separating = FindSeparating.NONE, int matches = 3)
         {
             var newCombines = new List<List<MItem>>();
             ...
             for (var col = 0; col < fieldData.maxCols; col++)
             {
                 for (var row = 0; row < fieldData.maxRows; row++)
                 {
                     ...
                     {
                         if (!countedSquares.ContainsValue(GetSquare(col, row).MItem))
                         {
         ...
         public List<MItem> GetRandomItems(int count)
         {
             var list = GetItems(true);

             var list2 = new List<MItem>();
         ...
         /// <returns></returns>
         public List<MItem> GetItems(bool onlySimple = false, MItem[] exceptItems = null, bool nonDestroying = true)
         {
             if (exceptItems == null) exceptItems = new MItem[] { };

             var items = squaresArray.Where(i => i?.MItem != null).Select(i => i.MItem).Except(exceptItems);
             if (onlySimple)
         ...
         /// </summary>
         /// <returns></returns>
         public List<MItem> GetJustCreatedItems()
         {
             return FindObjectsOfType<MItem>().Where(i => i.JustCreatedItem && i.needFall && !i.falling && !i.GetItemInterfaces()[0].IsStaticOnStart()).ToList();
         }
         ...
         /// </summary>
         /// <returns></returns>
         public List<MItem> GetItemsFromBottomOrder()
         {
             ...
             foreach (var seq in list)
             {
                 ...
                 foreach (var sq in seq)
                 {
                     if (sq.MItem != null)
                     {
                         sq.MItem.orderInSequence = order;
                         items.Add(sq.MItem);
                     }
                     order++;
                 }
         ...
         /// </summary>
         /// <returns></returns>
         public List<MItem> GetAllExtaItems()
         {
             var list = new List<MItem>();
             foreach (var square in squaresArray)
             {
                 if (square.MItem != null && square.MItem.currentType != ItemsTypes.NONE && square.MItem.Combinable)
                 {
                     list.Add(square.MItem);
                 }
             }
         ...
         {
             return squaresArray.Where(i => i.IsFree() && i.MItem == null && !i.IsHaveSolidAbove() && i.linkedEnterSquare && (i.enterSquare.isEnterPoint)).ToArray();
         }
         ...
         /// <returns></returns>
         public MItem[] GetItemsByColor(int color)
         {
         ...
         /// </summary>
         /// <returns></returns>
         public MItem[] GetLonelyItemsOrCage()
         {
             return squaresArray.Where(i => i.MItem != null).Where(i => !i.GetAllNeghborsCross().Any() || !i.CanGoOut()).Select(i => i.MItem).ToArray();
         }
