...
public class IColorableComponent : MonoBehaviour /* , IPoolable */
{
    ...
    private void Awake()
    {
        _mItemComponent = GetComponent<MItem>();
    ...
    private void OnEnable()
    {
        ...
        if (_mItemComponent && !_mItemComponent.Combinable) color = GetHashCode();
    }

    // [HideInInspector]
    // public bool colorGenerated;
    public bool RandomColorOnAwake = true;
    private MItem _mItemComponent;
    ...
    {
        if (_color < 0 || _color >= GetSprites(MLevelManager.THIS.currentLevel).Length) return;
        // colorGenerated = true;
        var component = _mItemComponent;
        ...
        if (GetSprites(MLevelManager.THIS.currentLevel).Length > 0 && directSpriteRenderer)
            directSpriteRenderer.sprite = GetSprites(MLevelManager.THIS.currentLevel)[_color];
        foreach (var i in iColorChangables) i.OnColorChanged(_color);
    }

    public void RandomizeColor()
    {
        color = ColorGenerator.GenColor(_mItemComponent.square);
        SetColor(color);
