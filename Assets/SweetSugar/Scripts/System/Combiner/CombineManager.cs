...
namespace SweetSugar.Scripts.System.Combiner
{
    ...
    public class CombineManager
    {
        ...
        Dictionary<MItem, Combine> dic = new Dictionary<MItem, Combine>();
        Dictionary<MItem, Combine> dicPredict = new Dictionary<MItem, Combine>();
        ...
        public List<List<MItem>> GetCombinedItems(FieldBoard field, bool setNextItemType = false)
        {
            var combinedItems = new List<List<MItem>>();
            ...
            foreach (var cmb in combines)
            {
                if (cmb.nextType != ItemsTypes.NONE)
                {
                    ...
                    var draggedItem = MLevelManager.THIS.lastDraggedMItem;
                    if (draggedItem)
                    {
                        if (draggedItem.color != item.color)
                            draggedItem = MLevelManager.THIS.lastSwitchedMItem;
        ...
        public List<Combine> GetCombines(FieldBoard field, out List<Combine> allFoundCombines, ItemsTypes _prioritiseItem = ItemsTypes.NONE)
        {
            ...
            for (var row = 0; row < maxRows; row++)
            {
                ...
                for (var col = 0; col < maxCols; col++)
                {
                    ...
                    {
                        CheckMatches(square.MItem, color, ref combine);

                        color = square.MItem.color;
                    }
                }
            }
            vChecking = true;
            ...
            for (var col = 0; col < maxCols; col++)
            {
                ...
                {
                    var square = field.GetSquare(col, row);
                    if (IsSquareNotNull(square) && !square.MItem.falling && !square.MItem.destroying)
                    {
        ...
        public List<Combine> CheckCombines(Dictionary<MItem, Combine> d, List<Combine> foundCombines)
        {
            ...
            foreach (var comb in foundCombines)
            {
                if (combines.IndexOf(comb) < 0 /*&& IsCombineMatchThree(comb)*/ && countedCombines.IndexOf(comb) < 0 && comb.items.Any())
                {
                    ...
                    {
                        comb.items = foundBonusCombine.Where(i => i.item).Select(i => i.mItemRef).ToList();
                        combines.Add(comb);
                    }
        ...
        Combine MergeCombines(Combine comb1, Combine comb2)
        {
            ...
            return new Combine().ConvertToCombine(foundBonusCombine?.Select(i => i.mItemRef).ToList());
        }
        ...
        void CheckMatches(MItem mItem, int color, ref Combine combine)
        {
            if(!mItem.destroying && !mItem.falling)
            {
                combine = FindCombine(mItem);
                AddItemToCombine(combine, mItem, dic, match3Combines);
            }
        }


        void AddItemToCombine(Combine combine, MItem mItem, Dictionary<MItem, Combine> d, List<Combine> lCombines)
        {
            //		if (lCombines.Any(i => i==combine)) return;
            combine.AddingMItem = mItem;
            d[mItem] = combine;

            if (IsCombineMatchThree(combine))
            {
                combine.color = mItem.color;
        ...
        Combine FindCombine(MItem mItem)
        {
            Combine combine = null;
            var leftItem = mItem.GetLeftItem();
            if (CheckColor(mItem, leftItem) && !vChecking)
                combine = FindCombineInDic(leftItem, dic);
            if (combine != null)
                return combine;
            var topItem = mItem.GetTopItem();
            if (CheckColor(mItem, topItem) && vChecking)
        ...
        Combine FindCombineInDic(MItem mItem, Dictionary<MItem, Combine> d)
        {
            Combine combine;
            if (d.TryGetValue(mItem, out combine))
            {
                return combine;
            }
            return new Combine();
        ...
        bool CheckColor(MItem mItem, MItem nextMItem)
        {
            if (nextMItem && nextMItem.Combinable && mItem.Combinable)
            {
                if (nextMItem.color == mItem.color && nextMItem.currentType != ItemsTypes.MULTICOLOR && nextMItem.currentType != ItemsTypes.INGREDIENT)
                    return true;
            }
            return false;
        }
    ...
    public class Combine
    {
        ...
        [FormerlySerializedAs("triggerItem")] public MItem triggerMItem;
        public Vector2 dir;
        [FormerlySerializedAs("mainSearchItem")] public MItem mainSearchMItem;

        public MItem AddingMItem
        {
            get
            {
                return _addingMItem;
            ...
            {
                _addingMItem = value;
                color = _addingMItem.color;
                if (CompareColumns(_addingMItem))
                {
                    if ((int)latestItemPositionH.y != _addingMItem.square.row && latestItemPositionH.y > -1)
                        hCount = 0;
                    hCount++;
                    latestItemPositionH = new Vector2(_addingMItem.square.col, _addingMItem.square.row);

                }
                else if (CompareRows(_addingMItem))
                {
                    if ((int)latestItemPositionV.x != _addingMItem.square.col && latestItemPositionV.x > -1)
                        vCount = 0;
                    vCount++;
                    latestItemPositionV = new Vector2(_addingMItem.square.col, _addingMItem.square.row);
        ...
        bool CompareRows(MItem mItem)
        {
            if (items.Count > 0)
            {
                if (mItem.square.row != PreviousItem().square.row)
                    return true;
            }
            else
                return true;
        ...
        bool CompareColumns(MItem mItem)
        {
            if (items.Count > 0)
            {
                if (mItem.square.col != PreviousItem().square.col)
                    return true;
            }
            else
                return true;
        ...
        public Combine ConvertToCombine(List<MItem> list)
        {
