...
namespace SweetSugar.Scripts.System.Pool
{
    ...
    public class ObjectPooler : MonoBehaviour
    {
        ...
        {
            if (MLevelManager.THIS.DebugSettings.FallingLog) DebugLogKeeper.Log(obj + " pooled", DebugLogKeeper.LogType.Falling );
            obj.SetActive(false);
//        Destroy(obj);
            MItem mItem = obj.GetComponent<MItem>();
            if (mItem != null)
            {
                if(mItem.transform.childCount>0)
                    mItem.transform.GetChild(0).localScale = Vector3.one;
            }
        }
        ...
        public GameObject GetPooledObject(string tag, Object activatedBy=null, bool active = true, bool canBeActive = false)
        {
            ...
            for (int i = 0; i < pooledObjects.Count; i++)
            {
                ...
                {
                    MItem mItem = pooledObjects[i].GetComponent<MItem>();
                    if(mItem && mItem.canBePooled )
                        obj = pooledObjects[i];
                    else if(!mItem )
                        obj = pooledObjects[i];
                    if(obj) break;
            ...
            if (MLevelManager.THIS.DebugSettings.FallingLog) DebugLogKeeper.Log(obj + " unpooled by " + activatedBy, DebugLogKeeper.LogType.Falling );
