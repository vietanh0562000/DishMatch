...
//types of items
    ...
    public class MItem : ItemMonoBehaviour, ISquareItemCommon, IField, IColorChangable, IMarmaladeTargetable
    {
        ...
        public ItemsTypes NextType
        {
            ...
            set
            {
                if (MLevelManager.THIS.DebugSettings.BonusCombinesShowLog)
...
//            sprRenderer.FirstOrDefault().sortingOrder = 2;
        ...
        public void CheckPlusFive()
        {
            ...
            if (MLevelManager.THIS.gameStatus == GameState.PrepareGame) range = Random.Range(0, 5);
            if (currentType == ItemsTypes.NONE && MLevelManager.THIS.mLevelData.limitType == LIMIT.TIME)
            {
        ...
        public void GenColor()
        {
            ...
            MLevelManager.THIS.mLevelData.SetItemTarget(this);
        }

        private void GenMarmalade()
        {
            if (MLevelManager.THIS.enableMarmalade && Random.Range(0, 10) == 0 &&
                MLevelManager.THIS.gameStatus == GameState.Playing && currentType == ItemsTypes.NONE && !tutorialItem)
            {
                var marmaladeCount = MLevelManager.THIS.field.GetItems()?.Where(i => i.currentType == ItemsTypes.MARMALADE)
                                         ?.Count() ?? 0;
            }
        }
        ...
        private void ResetDrag()
        {
            ...
            neighborSquare = null;
            _switchMItem = null;
            MLevelManager.THIS.DragBlocked = false;
        }
        ...
        private void Update()
        {
            ...
            if (dragThis && !MLevelManager.THIS.findMatchesStarted)
            {
            ...
            if(!falling && square && MLevelManager.GetGameStatus() != GameState.RegenLevel && field == MLevelManager.THIS.field)
                CheckSquareBelow();
        }
        ...
        public void SwitchDirection(Vector3 delta)
        {
            ...
            {
                MLevelManager.THIS.DragBlocked = true;
                _switchMItem = null;
                ...
                if (neighborSquare != null)
                    _switchMItem = neighborSquare.MItem;
                if (_switchMItem != null)
                {
                    if (_switchMItem.square.GetSubSquare().CanGoOut())
                        MLevelManager.THIS.StartCoroutine(Switching());
                    else if (((currentType != ItemsTypes.NONE || _switchMItem.currentType != ItemsTypes.NONE) && (currentType != ItemsTypes.INGREDIENT && _switchMItem.currentType != ItemsTypes.INGREDIENT)) &&
                             _switchMItem.square.GetSubSquare().CanGoOut())
                        MLevelManager.THIS.StartCoroutine(Switching());
                    else
                        ResetDrag(); //1.6.1
                }
        ...
        private IEnumerator Switching()
        {
            if (switchDirection != Vector3.zero && neighborSquare)
            {
                ...
                MItem[] switchingItems = { this, _switchMItem };
                var backMove = false;
                neighborSquare.MItem = this;
                square.MItem = _switchMItem;
                _switchMItem.square = backupSquares[0];
                square = backupSquares[1];
                var combines2 = GetMatchesAround().Concat(backupSquares.First().MItem.GetMatchesAround());
                ...
                while (distCovered < 1)
                {
                    ...
                    if(currentType == ItemsTypes.MULTICOLOR || _switchMItem.currentType == ItemsTypes.MULTICOLOR)
                    ...
                    else
                    {
                        transform.position = Vector3.Lerp(startPos, neighborSquare.transform.position + Vector3.back * 0.3f,
                            distCovered);
                        _switchMItem.transform.position = Vector3.Lerp(neighborSquare.transform.position + Vector3.back * 0.2f,
                            startPos, distCovered);
                    }
                    yield return new WaitForFixedUpdate();
                }
                var list = new[] {this, _switchMItem};

                if ((!combines2.Any() && !IsSwitchBonus() && MLevelManager.THIS.ActivatedBoost.type != BoostType.FreeMove) &&
                                                                                                      ...
                                                                                                      && list.Any(i=>!i.CombinableWithBonus))) 
                {
                    ...
                    _switchMItem.square = backupSquares[1];
                    neighborSquare.MItem = _switchMItem;
                    square.MItem = this;
                    backMove = true;
                ...
                {
                    if (MLevelManager.THIS.ActivatedBoost.type != BoostType.FreeMove)
                    {
                        if (MLevelManager.THIS.mLevelData.limitType == LIMIT.MOVES)
                            MLevelManager.THIS.mLevelData.limit--;
                        MLevelManager.THIS.moveID++;
                    }

                    if (MLevelManager.THIS.ActivatedBoost.type == BoostType.FreeMove)
                        MLevelManager.THIS.ActivatedBoost = null;

                    MLevelManager.THIS.lastDraggedMItem = this;
                    MLevelManager.THIS.lastSwitchedMItem = _switchMItem;
                }

                if (!backMove)
                {
                    BonusesAnimation(this, _switchMItem);
                    yield return new WaitWhile(() => MLevelManager.THIS.StopFall);
                    Check(this, _switchMItem);
                    foreach (var combine in combines2)
                    {
                        if (combine.nextType != ItemsTypes.NONE)
                        {
                            if (combine.color == MLevelManager.THIS.lastDraggedMItem.color)
                                MLevelManager.THIS.lastDraggedMItem.NextType = combine.nextType;
                            else if (combine.color == MLevelManager.THIS.lastSwitchedMItem.color)
                                MLevelManager.THIS.lastSwitchedMItem.NextType = combine.nextType;
                        }
                    }
                    ...
                    MLevelManager.THIS.mLevelData.GetTargetsByAction(CollectingTypes.ReachBottom).ForEachY(i => i.CheckBottom());
                    CheckAndChangeTypes();
                }
                ...
                while (distCovered < 1 && backMove)
                {
                    ...
                    transform.position = Vector3.Lerp(neighborSquare.transform.position + Vector3.back * 0.3f, startPos,
                        distCovered);
                    _switchMItem.transform.position = Vector3.Lerp(startPos,
        ...
        private bool NotContainsBoth(ItemsTypes type1, ItemsTypes type2) => (currentType != type1 && _switchMItem.currentType != type2);

        private bool ContainsBoth(ItemsTypes type1, ItemsTypes type2) =>
            currentType == type1 && _switchMItem.currentType == type2 ||
            currentType == type2 && _switchMItem.currentType == type1;
        ...
        public IEnumerator DirectionAnimation(Action callback)
        {
            ...
            {
                if (itemPrefabObject == null || !square || falling || needFall || destroying || destroyNext || MLevelManager.THIS.DragBlocked)
                {
        ...
        public virtual void Check(MItem item1, MItem item2)
        {
        ...
        public void BonusesAnimation(MItem item1, MItem item2)
        {
        ...
        public List<Combine> GetMatchesAround()
        {
            ...
            var combineManager = MLevelManager.THIS.CombineManager;
            var dic = new Dictionary<MItem, Combine>();
            ...
            MLevelManager.THIS.combo += combines2.Count;
            return combines2;
        }
        ...
        {
            if (((currentType == ItemsTypes.MULTICOLOR || _switchMItem.currentType == ItemsTypes.MULTICOLOR) &&
                 (Combinable || _switchMItem.Combinable)) || (currentType == ItemsTypes.MULTICOLOR && _switchMItem.currentType == ItemsTypes.MULTICOLOR))
                return true;
            if (currentType > 0 && _switchMItem.currentType > 0 && (currentType != ItemsTypes.INGREDIENT && _switchMItem.currentType != ItemsTypes.INGREDIENT) && Combinable && _switchMItem.Combinable)
                return true;
            if (!Combinable && !_switchMItem.Combinable && currentType != ItemsTypes.MULTICOLOR && _switchMItem.currentType != ItemsTypes.MULTICOLOR)
                return false;
            return false;
        }
...
//        if (LevelManager.THIS.StopFall) return;
            ...
            {
                if (MLevelManager.THIS.DebugSettings.FallingLog) DebugLogKeeper.Log(name + " start fall, target - " + square, DebugLogKeeper.LogType.Falling);
                falling = true;
        ...
        private void StartFallingSides()
        {
            ...
            if (MLevelManager.THIS.DebugSettings.FallingLog) DebugLogKeeper.Log(name + " start side fall, target - " + square, DebugLogKeeper.LogType.Falling);
            falling = true;
        ...
        {
            var waypoints = MLevelManager.THIS.field.GetWaypoints(previousSquare, targetSquare, new List<Square>());
            if (waypoints.Any()) return waypoints;
            if (!targetSquare.isEnterPoint)
                waypoints = MLevelManager.THIS.field.GetWaypoints(targetSquare.enterSquare, targetSquare, new List<Square>());
            if (waypoints.Any()) return waypoints;
...
//        _square.teleport.EnableMask(false);
        ...
        private IEnumerator FallingCor(List<Waypoint> waypoints, bool animate, Action callback = null)
        {
            ...
            float speed = MLevelManager.THIS.fallingCurve.Evaluate(0);
            int sideFall = 2;
...
//            sideFall = 2;
            ...
            for (var i = 0; i < waypoints.Count; i++)
            {
                ...
                {
                    if(MLevelManager.THIS.StopFall)
                    {
                        fallStopped = true;
                        pauseTime = Time.time;
                    }
                    yield return new WaitWhile(() => MLevelManager.THIS.StopFall);
                    if(fallStopped && !MLevelManager.THIS.StopFall)
                    {
                        fallStopped = false;
                    ...
                    speed = MLevelManager.THIS.fallingCurve.Evaluate(Time.time - startTimeGlobal ) * sideFall;
                    ...
                    if(!hit2D || !hit2D.transform.GetComponent<MItem>().falling 
                    ...
                    if (MLevelManager.THIS.DebugSettings.FallingLog)
...
//                    break;
                    ...
                    if (fracJourney >= 0.5f)
                    {
                        ...
                        if (squareNew != null && squareNew.MItem == null && squareNew.IsFree())
                        {
                            JustCreatedItem = false;
                            square.MItem = null;
                            squareNew.MItem = this;
                            if (MLevelManager.THIS.DebugSettings.FallingLog) DebugLogKeeper.Log(name + " change square from " + square + " to " + squareNew, DebugLogKeeper.LogType.Falling);
                            square = squareNew;
            ...
            JustCreatedItem = false;
            if (previousSquare?.MItem == this) previousSquare.MItem = null;
...
//        transform.position = destPos;// square.transform.position;
            ...
            yield return new WaitForSeconds(MLevelManager.THIS.waitAfterFall);
            CheckSquareBelow();
            ...
            if (!needFall)
            {
                ...
                if(!square.GetAllNeghborsCross().Any(i => (i.MItem && i.MItem.falling && i.MItem.color == color)))
                {
                    ...
                    if(square.type == SquareTypes.JellyBlock)
                        MLevelManager.THIS.mLevelData.GetTargetObject().CheckSquares(selectMany.Select(i => i.square).ToArray());
                }
                OnStopFall();
            }
        }
        ...
        {
            return (hit2D.transform.GetComponent<MItem>().square.transform.position - transform.position).magnitude > (square.transform.position-transform.position).magnitude 
                   && square.direction == hit2D.transform.GetComponent<MItem>().square.direction;
        }
        ...
        {
            falling = false;
            if (MLevelManager.THIS.DebugSettings.FallingLog) DebugLogKeeper.Log(name + " stop fall " + square + " pos " + transform.position, DebugLogKeeper.LogType.Falling);
            if (square.MItem != this) DestroyBehaviour();
        }
        ...
        public void CheckNearEmptySquares()
        {
            ...
            if (!square.CanGoOut() || MLevelManager.THIS.StopFall) return;
        ...
        {
            if (!checkingSquare.IsNone() && checkingSquare.CanGoInto() && checkingSquare.MItem == null && !falling)
            {
                if ((checkingSquare.MItem == null || (bool)checkingSquare.MItem?.destroying ||
                     (bool)checkingSquare.MItem?.falling) && checkingSquare.GetItemAbove() == null)
                {
                    square.MItem = null;
                    previousSquare = square;
                    checkingSquare.MItem = this;
        ...
        public void ChangeType(Action<MItem> callback=null, bool dontDestroyForThisCombine = true, bool destroyBlock = true)
        {
            if ( NextType != ItemsTypes.NONE )
            {
        ...
        IEnumerator ChangeTypeCor(Action<MItem> callback=null,bool dontDestroyForThisCombine = true, bool destroyBlock = true)
        {
            ...
            MItem newMItem = square.GenItem(false, NextType, color);
            newMItem.dontDestroyForThisCombine = dontDestroyForThisCombine;
            newMItem.transform.position = transform.position;
            newMItem.debugType = newMItem.currentType;
            if (MLevelManager.THIS.DebugSettings.BonusCombinesShowLog)
                DebugLogKeeper.Log("set " + NextType + " " + name + " to " + newMItem.name,DebugLogKeeper.LogType.BonusAppearance);
            List<Waypoint> waypoints = new List<Waypoint>();
            newMItem.CheckSquareBelow();
            newMItem.square.MItem = newMItem;
            newMItem.transform.position = newMItem.square.transform.position;
            NextType = ItemsTypes.NONE;
            if(destroyBlock)
                square.DestroyBlock();
            if (square && this == square.MItem)
                square.MItem = null;
            if(!destroying)
                ObjectPooler.Instance.PutBack(gameObject);
            callback?.Invoke(newMItem);
            yield return new WaitForSeconds(0.3f);
        }
        ...
        void OnNextMove()
        {
            ...
            MLevelManager.OnTurnEnd -= OnNextMove;
        }

        [FormerlySerializedAs("explodedItem")] [HideInInspector] public MItem explodedMItem;
        ...
        /// <param name="explodedMItem></param>
        /// <param name="explEffect"></param>

        public void DestroyItem(bool showScore = false, bool particles = true, MItem explodedMItem = null, bool explEffect = false)
        {
            ...
            this.explodedMItem = explodedMItem;
            if (!explodedMItem && (!marmaladeTarget || (!marmaladeTarget?.activeSelf ?? false)) && (dontDestroyForThisCombine || dontDestroyOnThisMove /* || NextType != ItemsTypes
        .NONE*/ ||/* falling 
        ||*/ needFall) || !Explodable)
            {
                StopDestroy();
            ...
            if(!destroycoroutineStarted)
                StartCoroutine(DestroyCor(showScore, particles, explodedMItem, explEffect));
        }

        private IEnumerator DestroyCor(bool showScore = false, bool particles = true, MItem explodedMItem = null, bool explEffect = false)
        {
            destroycoroutineStarted = true;
            if (explodedMItem != null)
                _switchMItem = explodedMItem;
            SoundBase.Instance
            ...
            if (MLevelManager.THIS.mLevelData.limitType == LIMIT.TIME && plusTimeObj)
            {
                plusTimeObj.GetComponent<PlusFiveBonus>().Destroy();
            }

            if (showScore)
                MLevelManager.THIS.ShowPopupScore(scoreForItem, transform.position, color);
            MLevelManager.Score += scoreForItem;
            MLevelManager.THIS.CheckStars();
            MLevelManager.THIS.mLevelData.GetTargetObject().CheckItems(new[] { this });
        ...
        {
            if (MLevelManager.THIS.DebugSettings.DestroyLog) DebugLogKeeper.Log("Stop destroy " + GetInstanceID(),DebugLogKeeper.LogType.Destroying);
            destroying = false;
            destroyNext = false;
            destroycoroutineStarted = false;
        }
        ...
        public MItem DeepCopy()
        {
            var other = (MItem)MemberwiseClone();
            return other;
        }
