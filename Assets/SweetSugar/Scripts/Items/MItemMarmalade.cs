...
namespace SweetSugar.Scripts.Items
{
    ...
    /// Item marmalade
    /// </summary>
    public class MItemMarmalade : MItem, IItemInterface
    {
        // public bool Combinable;
        public bool ActivateByExplosion;
        public bool StaticOnStart;
        public bool noMarmaladeLaunch;
        [FormerlySerializedAs("secondItem")] public MItemMarmalade secondMItem;
        ...
        private void DestroySecond(MItem item1, MItem item2)
        {
            secondMItem?.DestroySecond(item1, item2);
            var switchItemType = item2?.currentType ?? ItemsTypes.NONE;
        ...
        private void CreateMarmalade(ItemsTypes itemsType)
        {
            foreach (var marmalade in marmalades)
            {
                ...
                if (GetParentItem().square?.type == SquareTypes.JellyBlock || MLevelManager.THIS.lastSwitchedMItem?.square?.type == SquareTypes.JellyBlock)
        ...
        {
            if(square?.MItem == this && MLevelManager.THIS.gameStatus != GameState.RegenLevel)
                square.MItem = null;
        }
