...
namespace SweetSugar.Scripts.GUI
{
    ...
    public class TargetGUIGroup : MonoBehaviour
    {
        ...
        private void OnLevelLoaded()
        {
            ...
            if (group != null)
            {
                if (MLevelData.THIS.IsTargetByNameExist("JellyBlock"))
        ...
        {
            yield return new WaitUntil(() => MLevelManager.THIS.levelLoaded);
            yield return new WaitUntil(() => MLevelManager.THIS.mLevelData.GetTargetSprites().Length > 0);
        ...
        void SetTargets()
        {
            MLevelData mLevelData = MLevelManager.THIS.mLevelData;
            SetDescription(MLevelManager.THIS.mLevelData.GetFirstTarget(true)?.GetDescription());
            var targets = mLevelData.GetTargetContainersForUI();
            if (transform.parent.parent.parent.name == "PreFailed")
            {
                targets = mLevelData.GetTargetCounters().Where(i => !i.IsTotalTargetReached()).ToArray();
            }
