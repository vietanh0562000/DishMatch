    ...
    public class TargetCounter
    {
            ...
            TargetObject targetObject)
        {
            ...
            if(argTargetLevel.name == "Stars" && targetObject.ShowTheScoreForStar.ShowTheScore)
            {
                ...
                if (targetObject.CountDrawer.count == 1) count = MLevelData.THIS.star1; 
                if (targetObject.CountDrawer.count == 2) count = MLevelData.THIS.star2; 
                if (targetObject.CountDrawer.count == 3) count = MLevelData.THIS.star3; 
            }
        ...
        {
            int countJelly;
            countALlsquares = MLevelManager.THIS.fieldBoards.Sum(i => i.GetSquares().Where(x => x.IsAvailable()).Count());
            int countALl;
            if(!subLevel)
            {
                countJelly = MLevelManager.THIS.fieldBoards.Sum(i => i.GetSquares().Sum(x => x.GetSpriteRenderers().Distinct().Sum(y => extraObjects.Count(z => z == y.sprite))));
                countALl = MLevelManager.THIS.fieldBoards.Sum(i => i.GetSquares().Where(x => x.IsAvailable()).Count());
            }
            else
            {
                countJelly = MLevelManager.THIS.field.GetSquares().Sum(x => x.GetSpriteRenderers().Sum(y => extraObjects.Count(z => z == y.sprite)));
                countALl = MLevelManager.THIS.field.GetSquares().Where(x => x.IsAvailable()).Count();
            }
        ...
        private int GetSquareTargetCount(bool subLevel)
        {
            ...
            return GameObject.FindObjectsOfType<TargetComponent>().Where(i=>i.GetComponent<IField>().GetField()==MLevelManager.THIS.field).Sum(x => x.GetComponent<ISquareItemCommon>()
        ...
        public void OnCheckTarget(GameObject obj, Sprite spr)
        {
            ...
            if (objects.Count() > 0 && count > 0)
            {
                ...
                if (MLevelManager.THIS.animateItems.Any(i => square != null && i.linkObjectHash == square.hashCode)) return;
                // if(square)
                // {
            ...
            {
                if(!showScores) count = Savecount - MLevelManager.THIS.stars;
                else count = Savecount - MLevelManager.Score;
                if (count < 0) count = 0;
            }
        }
        ...
        public void changeCount(int i)
        {
            ...
            if (count < 0) count = 0;
            preCount = count;
            if(!MLevelManager.THIS.DragBlocked && count == 0) MLevelManager.THIS.CheckWinLose();
        }
        ...
        public virtual bool IsTargetReachedSublevel()
        {
            ...
            if (targetLevel.collectAction == CollectingTypes.ReachBottom && MLevelManager.THIS.field.IngredientsByEditor)
            {
                if (MLevelManager.THIS.field.GetItems().Count(i => i.currentType == ItemsTypes.INGREDIENT) == 0) return true;
            }
        ...
        public void CheckTarget(MItem[] items)
        {
        ...
        {
            var sqList = MLevelManager.THIS.field.GetBottomRow();
                foreach (var hItem in sqList)
                {
                    if (hItem.MItem == null || hItem.MItem.falling) continue;
                    var obj = hItem.MItem;
