...
namespace SweetSugar.Scripts.System.Combiner
{
    ...
    public class BonusItemPrediction
    {
        ...
        public static List<MItem> IsItemPredicted(ItemsTypes itemType)
        {
            Debug.Log("prediction bonus..");
            List<MItem> predicted;
        ...
        private static List<MItem> PridictCombines(FieldBoard field, bool right, ItemsTypes itemType)
        {
            var combineManager = MLevelManager.THIS.CombineManager;

            for (var i = 0; i < field.squaresArray.Count(); i++)
            {
                var item = field.squaresArray[i].MItem;
                MItem item1 = null;
                if (right)
                    item1 = (item?.square?.GetNeighborRight())?.MItem;
                else
                    item1 = (item?.square?.GetNeighborBottom())?.MItem;
