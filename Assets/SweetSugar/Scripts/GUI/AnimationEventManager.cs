...
//            SoundBase.Instance.PlayOneShot(SoundBase.Instance.gameOver[0]);
            ...
            if (name == "GemsShop")
            {
                ...
                for (var i = 0; i < MLevelManager.THIS.gemsProducts.Count; i++)
                {
                    var item = tr[i];
                    item.Find("Count").GetComponent<TextMeshProUGUI>().text = "" + MLevelManager.THIS.gemsProducts[i].count;
                    item.Find("Buy/Price").GetComponent<TextMeshProUGUI>().text = "" + MLevelManager.THIS.gemsProducts[i].price;
                }
            }
        ...
        public void OnFinished()
        {
            ...
            if (name == "MenuPause")
            {
                if (MLevelManager.THIS.gameStatus == GameState.Playing)
                    MLevelManager.THIS.gameStatus = GameState.Pause;
            }

            if (name == "MenuFailed")
            {
                if (MLevelManager.Score < MLevelManager.THIS.mLevelData.star1)
                {
                    TargetCheck(false, 2);
                }
                else
            ...
            {
                CloseMenu();
                MLevelManager.THIS.gameStatus = GameState.Tutorial;
                if(MLevelManager.THIS.mLevelData.limitType == LIMIT.TIME) SoundBase.Instance.PlayOneShot(SoundBase.Instance.timeOut);
        ...
        {
            if (name == "PreFailed" && MLevelManager.THIS.gameStatus != GameState.Playing)
            {
        ...
        {
            for (var i = 1; i <= MLevelManager.THIS.stars; i++)
            {
...
//            LevelManager.THIS.gameStatus = GameState.Map;
                PlayerPrefs.SetInt("OpenLevel", MLevelManager.THIS.currentLevel + 1);
            ...
            {
                MLevelManager.THIS.gameStatus = GameState.Map;
            }

            if (SceneManager.GetActiveScene().name == "game")
            {
                if (MLevelManager.THIS.gameStatus == GameState.Pause)
                {
                    MLevelManager.THIS.gameStatus = GameState.WaitAfterClose;

                }
            }

            if (gameObject.name == "Settings" && MLevelManager.GetGameStatus() != GameState.Map)
            {
                BackToMap();
            }
            else if (gameObject.name == "Settings" && MLevelManager.GetGameStatus() == GameState.Map)
        ...
        public void Play()
        {
            ...
            {
                CloseMenu();
                MLevelManager.THIS.gameStatus = GameState.Playing;
            }
        }
...
//        if (GetComponent<Animation>()["bannerFailed"].speed == 0)
            {
                if (InitScript.Gems >= MLevelManager.THIS.FailedCost)
                {
                    InitScript.Instance.SpendGems(MLevelManager.THIS.FailedCost);
