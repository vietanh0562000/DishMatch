...
namespace SweetSugar.Scripts
{
    ...
    public class AI : MonoBehaviour
    {
        ...
        /// The tip items
        /// </summary>
        private List<MItem> currentPreCombine = new List<MItem>();
        ...
        Square GetSquare(int row, int col, Square currentSquare)
        {
            ...
            if (v1.magnitude > 1) currentSquare = MLevelManager.THIS.field.GetSquare(currentSquare.GetPosition() + new Vector2(v1.x, v1.y * -1).normalized);
            if (currentSquare != null && currentSquare.IsDirectionRestricted(v1.normalized)) return null;
            return MLevelManager.THIS.GetSquare(col, row);
        }
        ...
        bool CheckSquare(Square square, int COLOR, bool moveThis = false)
        {
            if (square == null)
                return false;
            if(currentPreCombine==null) currentPreCombine= new List<MItem>();
            if (square.MItem != null)
            {
                if (CheckColorCondition(square, COLOR))
                {
                    if (moveThis && square.GetSubSquare().CanGoOut())
                    {
                        currentPreCombine.Add(square.MItem);
                        return true;
                    }
                    else if (!moveThis)
                    {
        ...
        {
            if(MLevelManager.THIS.gameStatus != GameState.Tutorial)
                return square.MItem.color == COLOR && square.MItem.Combinable;
            else
                return square.MItem.color == COLOR && square.MItem.Combinable && square.MItem.tutorialItem;

        }

        public List<MItem> GetCombine()
        {
        ...
        public IEnumerator CheckPossibleCombines()
        {
            ...
            maxRow = MLevelManager.THIS.mLevelData.maxRows;
            maxCol = MLevelManager.THIS.mLevelData.maxCols;

            //variable to check: are we got tip or not
            gotTip = false;

            //break, if the main scripts have not ready yet
            while (MLevelManager.THIS == null)
            {
                yield return new WaitForEndOfFrame();
            }
            //if game is not in Playing status - wait
            while (MLevelManager.THIS.gameStatus != GameState.Playing && MLevelManager.THIS.gameStatus != GameState.Tutorial)
            {
                yield return new WaitForEndOfFrame();
            }

            //if drag have not blocked and game status Playing - continue
            if (!MLevelManager.THIS.DragBlocked && (MLevelManager.THIS.gameStatus == GameState.Playing || MLevelManager.THIS.gameStatus == GameState.Tutorial))
            {
                currentPreCombine = new List<MItem>();
                ...
                if (itemSprites != null)
                    for (var COLOR = 0; COLOR < itemSprites.GetSprites(MLevelManager.THIS.currentLevel).Length; COLOR++)
                    {
                        if (changeTipAI && Random.Range(0, 2) == 1) continue;
                        for (var col = 0; col < MLevelManager.THIS.mLevelData.maxCols; col++)
                        {
                            for (var row = 0; row < MLevelManager.THIS.mLevelData.maxRows; row++)
                            {
                                var square = MLevelManager.THIS.GetSquare(col, row);
                                if (square?.GetSubSquare().CanGoOut() == false || square.MItem == null)
                                    continue;
                                ...
                                if (currentPreCombine.Count == 3 && square.CanGoInto() && square.GetSubSquare().CanGoOut())
                                {
                                    if (v > h && currentPreCombine[2].square.GetSubSquare().CanGoOut())
                                    {
                                        ...
                                        TipMItem = currentPreCombine[2];
                                        if (TipMItem.transform.position.x > currentPreCombine[0].transform.position.x)
                                            vDirection = Vector3.left;
                                        else
                                            vDirection = Vector3.right;
                                        showTip(currentPreCombine);
                                ...
                                else if (square.mItem && square.mItem.currentType != ItemsTypes.NONE && square.mItem.currentType!=ItemsTypes.TimeBomb && square.CanGoInto() && square.GetSubSquare()
                                .CanGoOut() &&
                                         square.MItem.Combinable &&
                                         square.MItem.CombinableWithBonus && currentPreCombine.Count < 3)
                                {
                                    currentPreCombine.Clear();
                                    List<Square> allNeghborsCross = square.GetAllNeghborsCross();
                                    var neibSquare = allNeghborsCross.FirstOrDefault(i => i.MItem && i.MItem.currentType != ItemsTypes.NONE && i.MItem.CombinableWithBonus );
                                    if (neibSquare && neibSquare.MItem && neibSquare.MItem.Combinable && neibSquare.CanGoOut())
                                    {
                                        TipMItem = square.MItem;
                                        vDirection = (neibSquare.MItem.transform.position - square.MItem.transform.position).normalized;
                                        currentPreCombine.Add(square.MItem);
                                        currentPreCombine.Add(neibSquare.MItem);
                                        showTip(currentPreCombine);
                                        yield break;
                                    }
                ...
                if (!gotTip)
                {
                    var mult = MLevelManager.THIS.field.GetItems().FirstOrDefault(i => i.currentType == ItemsTypes.MULTICOLOR);
                    if (mult && mult.square.CanGoOut())
                    {
                        var item = mult.square.GetAllNeghborsCross().FirstOrDefault(i => i.mItem != null);
                        if(item && item.CanGoOut())
                        {
                            currentPreCombine.Clear();
                            currentPreCombine.Add(item.mItem);
                ...
                if (!MLevelManager.THIS.DragBlocked && !MLevelManager.THIS.field.GetItems().Any(i => i.falling || i.destroying))
                {
                    if (!gotTip && !_debugSettings.AI)
                        MLevelManager.THIS.NoMatches();
                }
        ...
        private int GetCrossCombine(int row, int col, Square square, int COLOR, int v, ref int h)
        {
            ...
            if (row < maxRow - 1)
            {
                ...
                {
                    //1.6
                    if (square1.MItem != null)
                    {
                        if (CheckColorCondition(square1, COLOR))
                        {
                            vDirection = Vector3.up;
                            currentPreCombine.Add(square1.MItem);
                            v++;
                        }
                    }
                }
        ...
        void showTip(List<MItem> nextMoveItems)
        {
        ...
        //show tip coroutine
        IEnumerator showTipCor(List<MItem> nextMoveItems)
        {
            changeTipAI = false;
            gotTip = true;
            corCount++;
            ...
            if (MLevelManager.THIS.DragBlocked && !allowShowTip)
            {
                corCount--;
                yield break;
            }
            tipID = MLevelManager.THIS.moveID;
            //while (!LevelManager.This.DragBlocked && allowShowTip)
            //{
            yield return new WaitForSeconds(1);
            if (MLevelManager.THIS.DragBlocked && !allowShowTip && tipID != MLevelManager.THIS.moveID)
            {
                corCount--;
                yield break;
            }
            ...
            StartCoroutine(CheckPossibleCombines());
// #if UNITY_EDITOR
            if (MLevelManager.THIS?.gameStatus == GameState.Playing && (_debugSettings?.AI ?? false)&& currentPreCombine?.Count() > 0)
            {
                if (TipMItem && !TipMItem.destroying && TipMItem.gameObject.activeSelf && !MLevelManager.THIS.findMatchesStarted && !MLevelManager.THIS.DragBlocked) TipMItem.SwitchDirection
                (-THIS.vDirection);
                yield return new WaitForSeconds(1);
            ...
            for (var COLOR = 0; COLOR < 6; COLOR++)
            {
                for (var col = 0; col < MLevelManager.THIS.mLevelData.maxCols; col++)
                {
                    for (var row = 0; row < MLevelManager.THIS.mLevelData.maxRows; row++)
                    {
                        ...
                        {
                            var l = new MItem[currentPreCombine.Count];
