...
namespace SweetSugar.Scripts.TargetScripts
{
    ...
    public class SolidBlock : Target
    {
        ...
        {
            subTargetContainers[0].count = mLevelData.fields.Sum(x => x.levelSquares.Count(i =>  i.block == SquareTypes.SolidBlock ||  i.obstacle == SquareTypes.SolidBlock));
        }
        ...
        {
            var count = 0;
            var fieldBoards = MLevelManager.THIS.fieldBoards;
        ...
        public override void FulfillTarget<T>(T[] _items)
        {
            ...
            var pos = TargetGUI.GetTargetGUIPosition(MLevelData.THIS.GetFirstTarget(true).name);
